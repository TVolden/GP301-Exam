...
void main()
{
   ...
   vec4 albedo = texture(texture_diffuse1, fs_in.TexCoord);

   // ambient occlusion texture sampling adjusted with mix parameter
   float ambientOcclusion = texture(texture_ambient1, fs_in.TexCoord).r;
   ...
   vec3 N = texture(texture_normal1, fs_in.TexCoord).xyz;
   N = N * 2f - 1f;
   ...
   vec3 reflectionColor = texture(skybox, fs_in.InverseTBN * tangentReflect).rgb;
